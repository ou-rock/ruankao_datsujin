...
```text
L5 入口适配层      cli.py, web.py, web_app.py, web_actions.py, web_bootstrap.py, web_files.py, web_handlers.py, web_forms.py, web_page.py, web_page_style.py, shell scripts, .codex commands
...
| `web_page.py` | L5 | `domain`, `evolution`, `export_state`, `memory`, `rag`, `receipts`, `route_map`, `web_page_style`, `web_render` | 工作台首页页面适配器。负责首页数据装配与 HTML 页面组合。不处理 HTTP、写入动作或启动初始化。 |
...
web_page     -> domain, evolution, export_state, memory, rag, receipts, route_map, web_page_style, web_render
web_page_style -> -
...
`web_page.py` 是工作台首页页面适配器。它装配首页所需的只读数据，
调用 `web_render.py` 片段，生成完整首页 HTML；它不能处理 HTTP、
...
| 热点 | 证据 | 风险 | 优化方向 |
| --- | --- | --- | --- |
| `web_page.py` | 约 480 行，依赖 9 个内部模块 | 首页数据装配和表单布局仍在一个模块 | 后续拆 `web_page_sections.py` |
...
12. `web_page.py` 只负责工作台首页页面装配，可以依赖 `web_render.py`, `web_page_style.py` 和只读内层服务，不能处理 HTTP、写状态或执行启动初始化。
13. `web_app.py` 是工作台应用编排层，可以依赖内层模块，但不能处理 HTTP server 绑定。
14. `cli.py` 和 `web.py` 是组合根，可以依赖内层模块。
15. 除 `cli.py` 之外，任何模块不得依赖 `web.py`。
16. 内层模块不得依赖 `cli.py`, `web.py`, `web_actions.py`, `web_app.py`, `web_bootstrap.py`, `web_files.py`, `web_forms.py`, `web_handlers.py`, `web_page.py`, `web_page_style.py`, `.codex` 命令、shell scripts 或浏览器自动化工具。

## 下一轮架构优化顺序

1. 继续拆 `web_page.py`：样式常量已离开页面模块，下一步按区块拆页面结构。
