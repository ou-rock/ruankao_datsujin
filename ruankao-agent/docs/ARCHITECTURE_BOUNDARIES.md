...
L4 报告与页面层    dashboard.py, receipts.py, receipts_payload.py, receipts_render.py, receipts_sections.py, receipts_style.py, route_map.py, route_map_payload.py, route_map_render.py, route_map_style.py, evolution.py, learning.py, learning_cheko_templates.py, learning_content.py, learning_layout.py, learning_style.py, learning_templates.py, rag.py, rag_report.py, rag_report_style.py, web_labels.py, web_lists.py, web_render.py
...
| `web_render.py` | L4 | `domain`, `memory`, `storage`, `web_labels` | 工作台控制片段、RAG 面板和三题型概览渲染。可以依赖展示标签，不能处理 HTTP、打开数据库、写状态、保存标签映射或承载列表渲染。 |
...
| `web_page_sections.py` | L5 | `web_lists`, `web_page_forms`, `web_page_style`, `web_page_view`, `web_render` | 工作台首页结构适配器。负责首页 HTML shell、导航、今日闭环和只读展示区块。不读取数据库或处理 HTTP。 |
...
web_page_sections -> web_lists, web_page_forms, web_page_style, web_page_view, web_render
web_page_style -> -
web_page_view -> -
web_labels   -> domain
...
调用 `web_render.py` 控制片段和 `web_lists.py` 列表片段，组合首页 HTML shell、导航、今日闭环和只读展示区块；
...
| `web_page_sections.py` | 约 130 行，依赖 5 个内部模块 | 页面壳已变薄，仍聚合首页所有区块调用 | 保持只做布局编排，不回填表单细节 |
...
| `web_render.py` | 约 260 行，依赖 4 个内部模块 | 控制片段、RAG 面板和三题型概览仍集中，但列表已外移 | 后续按控制/RAG 面板继续拆 |
...
28. `web_lists.py` 只负责工作台列表 HTML 片段，可以依赖只读数据对象和 `web_labels.py`，不能处理 HTTP、写状态、打开数据库或承载表单区块。
29. `web_render.py` 只负责工作台控制片段、RAG 面板和三题型概览，可以依赖只读数据对象和 `web_labels.py`，不能处理 HTTP、写状态、保存标签映射或承载列表渲染。
30. `web_forms.py` 只负责工作台表单字段到 typed input 的转换，可以依赖 `domain.py`，不能依赖存储或渲染。
31. `web_handlers.py` 只负责 HTTP handler、路由分发和响应封装，可以依赖 `web_forms.py`，不能依赖业务模块或存储。
32. `web_actions.py` 只负责工作台副作用动作，可以依赖 `web_forms.py` 和内层服务，不能处理 HTTP 或渲染页面。
33. `web_bootstrap.py` 只负责工作台启动初始化，可以依赖 `learning.py`, `storage.py`, `vault.py`，不能处理 HTTP 或运行时业务。
34. `web_files.py` 只负责工作台文件读取和路径防护，不能打开数据库、渲染页面或处理 HTTP 状态码。
35. `web_page_style.py` 只保存工作台首页 CSS 常量，不能读取状态、生成表单或依赖业务模块。
36. `web_page_view.py` 只定义首页 view model，不渲染 HTML、不读取状态。
37. `web_page_forms.py` 只负责工作台首页表单区块 HTML，可以依赖 `web_lists.py`, `web_page_view.py` 和 `web_render.py`，不能解析 HTTP、读取数据库或触发写入动作。
38. `web_page_sections.py` 只负责工作台首页 shell、导航、今日闭环和只读展示区块，可以依赖 `web_lists.py`, `web_page_forms.py`, `web_page_view.py`, `web_render.py` 与 `web_page_style.py`，不能读取状态或处理 HTTP。
39. `web_page.py` 只负责工作台首页 view model 装配，可以依赖只读内层服务，不能处理 HTTP、写状态或执行启动初始化。
40. `web_app.py` 是工作台应用编排层，可以依赖内层模块，但不能处理 HTTP server 绑定。
41. `cli.py` 和 `web.py` 是组合根，可以依赖内层模块。
42. 除 `cli.py` 之外，任何模块不得依赖 `web.py`。
43. 内层模块不得依赖 `cli.py`, `web.py`, `web_actions.py`, `web_app.py`, `web_bootstrap.py`, `web_files.py`, `web_forms.py`, `web_handlers.py`, `web_page.py`, `web_page_forms.py`, `web_page_sections.py`, `web_page_style.py`, `web_page_view.py`, `.codex` 命令、shell scripts 或浏览器自动化工具。

## 下一轮架构优化顺序

1. 如果 `web_page_style.py` 需要继续演进，再考虑拆为静态 CSS 资源。
2. 如果 `web_render.py` 继续增长，再按控制/RAG 面板拆片段模块。
3. 如果 `web_lists.py` 继续增长，再拆卡片/练习/诊断列表。
4. 如果 `rag_progress.py` 继续增长，再按记忆/题型/练习拆 gate 策略。
5. 如果 `rag_documents.py` 接入 NotebookLM/Cheko 更多来源，再按来源拆构造器。
6. 如果 `learning_cheko_templates.py` 接入真实 Cheko 增量，再拆 task policy。
7. 如果 `receipts_sections.py` 继续增长，再拆 label/row helper。
8. 如果工作台表单继续增长，再拆 `web_page_forms.py`。
9. 最后拆 `cli.py`：让命令只绑定参数到应用服务。
