...
```text
L5 入口适配层      cli.py, web.py, web_app.py, web_actions.py, web_handlers.py, web_forms.py, shell scripts, .codex commands
...
| `web_app.py` | L5 | 多数模块, `web_actions`, `web_forms`, `web_render` | 工作台应用编排层。负责初始化、页面组合和文件读取。不处理 HTTP server 绑定。 |
...
web_app      -> dashboard, domain, evolution, export_state, learning, loop, memory, rag, receipts, route_map, storage, vault, web_actions, web_forms, web_render
web_forms    -> domain
...
| 热点 | 证据 | 风险 | 优化方向 |
| --- | --- | --- | --- |
| `web_app.py` | 约 1180 行，依赖 15 个内部模块 | 页面组合、文件读取和应用初始化仍在一起 | 后续拆 `web_files.py`, `web_page.py` |
...
9. `web_app.py` 是工作台应用编排层，可以依赖内层模块，但不能处理 HTTP server 绑定。
10. `cli.py` 和 `web.py` 是组合根，可以依赖内层模块。
11. 除 `cli.py` 之外，任何模块不得依赖 `web.py`。
12. 内层模块不得依赖 `cli.py`, `web.py`, `web_actions.py`, `web_app.py`, `web_forms.py`, `web_handlers.py`, `.codex` 命令、shell scripts 或浏览器自动化工具。

## 下一轮架构优化顺序

1. 继续拆 `web_app.py`：`web.py` 已降为启动入口，`web_actions.py` 已承接动作，下一步拆文件服务和页面组装。
