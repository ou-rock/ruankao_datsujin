...
```text
L5 入口适配层      cli.py, web.py, shell scripts, .codex commands
L4 报告与页面层    dashboard.py, receipts.py, route_map.py, evolution.py, learning.py
...
| `web.py` | L5 | 多数模块 | 本地 HTTP 工作台组合根。负责表单、路由和页面组合，不成为事实源。 |
...
vault        -> -
web          -> cheko, dashboard, domain, evolution, export_state, learning, loop, memory, rag, receipts, route_map, storage, study, vault
```
...
| 热点 | 证据 | 风险 | 优化方向 |
| --- | --- | --- | --- |
| `web.py` | 约 1900 行，依赖 14 个内部模块 | HTTP、表单、页面渲染和应用服务混在一起 | 后续拆成 `web_app.py`, `web_handlers.py`, `web_render.py`, `web_forms.py` |
...
5. `cli.py` 和 `web.py` 是组合根，可以依赖内层模块。
6. 除 `cli.py` 之外，任何模块不得依赖 `web.py`。
7. 内层模块不得依赖 `.codex` 命令、shell scripts 或浏览器自动化工具。

## 下一轮架构优化顺序

1. 先拆 `web.py`：把表单解析、路由处理、HTML 片段渲染分开。
