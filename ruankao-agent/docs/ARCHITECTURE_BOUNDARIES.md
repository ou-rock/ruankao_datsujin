...
| `web.py` | L5 | 多数模块, `web_render` | 本地 HTTP 工作台组合根。负责表单、路由和页面组合，不成为事实源。 |
...
vault        -> -
web          -> cheko, dashboard, domain, evolution, export_state, learning, loop, memory, rag, receipts, route_map, storage, study, vault, web_render
...
| 热点 | 证据 | 风险 | 优化方向 |
| --- | --- | --- | --- |
| `web.py` | 约 1500 行，依赖 15 个内部模块 | HTTP、表单、页面组合和应用服务仍在一起 | 已拆出 `web_render.py`；后续继续拆 `web_forms.py`, `web_handlers.py`, `web_app.py` |
...
6. `cli.py` 和 `web.py` 是组合根，可以依赖内层模块。
7. 除 `cli.py` 之外，任何模块不得依赖 `web.py`。
8. 内层模块不得依赖 `.codex` 命令、shell scripts 或浏览器自动化工具。

## 下一轮架构优化顺序

1. 继续拆 `web.py`：`web_render.py` 已承接 HTML 片段，下一步拆表单解析和路由处理。
