...
L4 报告与页面层    dashboard.py, receipts.py, route_map.py, evolution.py, learning.py, learning_content.py, learning_style.py, rag.py, rag_report.py, web_render.py
...
| `learning.py` | L4 | `domain`, `learning_content`, `learning_style` | 学习台 HTML 生成与 Cheko snapshot 文件编排。不能写 ruankao.db。 |
...
learning     -> domain, learning_content, learning_style
learning_content -> -
learning_style -> -
...
| `learning.py` | 约 560 行，依赖 3 个内部模块 | 样式和静态数据已外移，但 HTML 模板仍集中 | 后续按页面模板或资源写入编排继续拆 |
...
11. `learning.py` 可以依赖 `domain.py`, `learning_content.py` 和 `learning_style.py` 生成学习台资源，不能写 ruankao.db。
12. `web_render.py` 只负责工作台展示片段，可以依赖只读数据对象，不能处理 HTTP 或写状态。
13. `web_forms.py` 只负责工作台表单字段到 typed input 的转换，可以依赖 `domain.py`，不能依赖存储或渲染。
14. `web_handlers.py` 只负责 HTTP handler、路由分发和响应封装，可以依赖 `web_forms.py`，不能依赖业务模块或存储。
15. `web_actions.py` 只负责工作台副作用动作，可以依赖 `web_forms.py` 和内层服务，不能处理 HTTP 或渲染页面。
16. `web_bootstrap.py` 只负责工作台启动初始化，可以依赖 `learning.py`, `storage.py`, `vault.py`，不能处理 HTTP 或运行时业务。
17. `web_files.py` 只负责工作台文件读取和路径防护，不能打开数据库、渲染页面或处理 HTTP 状态码。
18. `web_page_style.py` 只保存工作台首页 CSS 常量，不能读取状态、生成表单或依赖业务模块。
19. `web_page_view.py` 只定义首页 view model，不渲染 HTML、不读取状态。
20. `web_page_forms.py` 只负责工作台首页表单区块 HTML，可以依赖 `web_page_view.py` 和 `web_render.py`，不能解析 HTTP、读取数据库或触发写入动作。
21. `web_page_sections.py` 只负责工作台首页 shell、导航、今日闭环和只读展示区块，可以依赖 `web_page_forms.py`, `web_page_view.py`, `web_render.py` 与 `web_page_style.py`，不能读取状态或处理 HTTP。
22. `web_page.py` 只负责工作台首页 view model 装配，可以依赖只读内层服务，不能处理 HTTP、写状态或执行启动初始化。
23. `web_app.py` 是工作台应用编排层，可以依赖内层模块，但不能处理 HTTP server 绑定。
24. `cli.py` 和 `web.py` 是组合根，可以依赖内层模块。
25. 除 `cli.py` 之外，任何模块不得依赖 `web.py`。
26. 内层模块不得依赖 `cli.py`, `web.py`, `web_actions.py`, `web_app.py`, `web_bootstrap.py`, `web_files.py`, `web_forms.py`, `web_handlers.py`, `web_page.py`, `web_page_forms.py`, `web_page_sections.py`, `web_page_style.py`, `web_page_view.py`, `.codex` 命令、shell scripts 或浏览器自动化工具。

## 下一轮架构优化顺序

1. 继续拆 `learning.py`：把 HTML 模板和资源写入编排分开。
2. 再拆 `receipts.py`：把日结 payload 构建和 HTML 渲染拆开。
