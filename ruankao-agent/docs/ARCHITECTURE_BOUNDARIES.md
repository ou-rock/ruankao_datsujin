...
L2 持久化层        storage.py, vault.py, export_state.py, import_state.py
...
| `storage.py` | L2 | `domain` | SQLite schema、仓储、数据行对象和写前滚动二进制备份。不能渲染 HTML 或写 Obsidian。 |
| `vault.py` | L2 | 无 | Obsidian 目录与 Markdown 生成，覆盖同步时保护用户批注段。不能读取业务状态，只接收调用方传入数据。 |
| `export_state.py` | L2 | `storage` | 导出本地 JSON 状态。不能承担业务决策。 |
| `import_state.py` | L2 | `storage` | 从 export-state JSON 快照重建 SQLite。不能修改 schema、生成学习建议或读取 Obsidian。 |
...
| `dashboard.py` | L4 | `theme` | 静态总图渲染。只接收快照，不读取数据库。 |
...
| `receipts_render.py` | L4 | `receipts_sections`, `receipts_style`, `theme` | 日结整页 HTML shell。只负责标题、CSS 注入、主题接入和 section 组装，不能读取数据库、调用 RAG、写文件或承载 section 细节。 |
...
| `route_map_render.py` | L4 | `route_map_style`, `theme` | 三题型覆盖 HTML 渲染。只接收 payload 并返回 HTML 字符串，可以依赖覆盖图 CSS 常量和共享主题，不能读取数据库或写文件。 |
| `route_map_style.py` | L4 | 无 | 三题型覆盖报告 CSS 常量。不能保存 payload、渲染 HTML 或生成文件。 |
| `evolution.py` | L4 | `receipts`, `theme` | 夜间进化草案。只能生成暂存计划，不直接改 live skill。 |
...
| `learning_layout.py` | L4 | `learning_content`, `learning_style`, `theme` | 学习台页面壳、通用卡片、题型标签和共享主题接入。不能保存课程内容、读取状态或生成文件。 |
...
| `rag_report.py` | L4 | `rag_observability`, `rag_report_style`, `rag_types`, `theme` | RAG payload、报告路径和 HTML 渲染。可以依赖 RAG 报告 CSS、共享主题和可观察契约，不能执行检索、排序或读取 SQLite。 |
| `rag_report_style.py` | L4 | 无 | RAG 报告 CSS 常量。不能保存 payload、渲染 HTML、执行检索或生成文件。 |
| `theme.py` | L4 | 无 | 共享日间/黑夜主题 CSS、切换按钮和脚本常量。不能读取状态、写文件、承载业务文案或处理 HTTP。 |
...
| `web_page_sections.py` | L5 | `theme`, `web_card_lists`, `web_diagnostic_lists`, `web_fronts`, `web_labels`, `web_page_forms`, `web_page_learning_forms`, `web_page_operations`, `web_page_style`, `web_page_view`, `web_rag_panel`, `web_status` | 工作台首页结构适配器。负责首页 HTML shell、导航、主题接入、今日闭环和只读展示区块。不读取数据库或处理 HTTP。 |
...
cli          -> cheko, dashboard, domain, evolution, export_state, import_state, learning, loop, notebooklm, principles, rag, receipts, route_map, semantic_ingest, study, sync_alerts, vault, web
dashboard    -> theme
domain       -> -
evolution    -> receipts, theme
...
learning_content -> -
learning_layout -> learning_content, learning_style, theme
learning_style -> -
...
rag_observability -> -
rag_report   -> rag_observability, rag_report_style, rag_types, theme
rag_report_style -> -
rag_types    -> domain
...
receipts_render -> receipts_sections, receipts_style, theme
...
route_map_render -> route_map_style, theme
route_map_style -> -
...
web_page_sections -> theme, web_card_lists, web_diagnostic_lists, web_fronts, web_labels, web_page_forms, web_page_learning_forms, web_page_operations, web_page_style, web_page_view, web_rag_panel, web_status
web_page_style -> -
web_page_view -> -
web_labels   -> domain
...
`theme.py` 是共享主题常量模块。它只提供早期主题脚本、黑夜/日间切换按钮、
切换脚本和跨页面 CSS 变量，不读取状态、不写文件、不承担业务内容。
...
- `import_state.py` 写出重建后的 SQLite 文件，不生成报告、不修改 schema。
...
- `theme.py` 只保存共享主题 CSS、按钮和脚本常量，不生成文件。
...
- `storage.py` 在写事务前生成 `data/backups/ruankao.db.bak.[1-7]`，不改变业务事实。
- `import_state.py` 可以从 `export-state` JSON 重建 SQLite，但不能引入新 schema 或读取 Obsidian。
...
| `web_page_sections.py` | 约 133 行，依赖 12 个内部模块 | 页面壳已变薄，仍聚合首页所有区块调用和共享主题接入 | 保持只做布局编排，不回填表单细节 |
...
| `cli.py` | 依赖 18 个内部模块 | 命令解析与命令执行容易变厚 | 后续拆成 `commands/*.py` 或应用服务层 |
...
| `receipts_render.py` | 约 30 行，依赖 3 个内部模块 | 整页 shell 已变薄，风险低 | 保持只做标题、CSS/主题注入和 section 组装 |
...
| `route_map_render.py` | 约 110 行，依赖 1 个样式模块和共享主题 | 三题型覆盖 HTML 和 CSS 已分离，风险中低 | 后续如改版，再拆 card helper |
...
| `learning_layout.py` | 约 40 行，依赖 3 个内部模块 | 页面壳、共享主题和通用片段独立，风险低 | 保持只做通用学习页渲染片段 |
...
| `rag_report.py` | 约 238 行，依赖 4 个内部模块 | RAG 报告 HTML、payload 和可观察契约已与 CSS/说明数据分离 | 后续如改版，再拆片段 helper |
| `rag_report_style.py` | 约 70 行，无内部依赖 | RAG 报告 CSS 已独立，风险低 | 保持只做 CSS 常量 |
| `theme.py` | 约 130 行，无内部依赖 | 共享主题集中，风险低 | 保持只做主题 CSS、按钮和脚本常量 |
...
| `import_state.py` | 约 210 行，依赖 1 个内部模块 | 双轨容灾恢复入口新增，直接重建 SQLite 文件 | 保持只消费 export-state JSON，不承载数据迁移或学习规则 |

## 允许依赖规则

1. `domain.py`, `notebooklm.py`, `theme.py`, `vault.py` 不依赖任何内部模块；`dashboard.py` 只允许依赖 `theme.py`。
2. `storage.py` 只依赖 `domain.py`，可以生成写前滚动二进制备份，不能渲染或读取 Obsidian。
3. `memory.py` 只依赖 `storage.py`。`import_state.py` 可以依赖 `storage.py` 初始化现有 schema 并重建 SQLite，不能修改 schema、处理 HTTP、生成报告或承载学习规则。
...
11. `rag_report.py` 只负责 RAG payload、报告路径和 HTML 渲染，可以依赖 `rag_observability.py`, `rag_report_style.py`, `rag_types.py` 和 `theme.py`，不能执行检索或排序；`rag_observability.py` 必须保持无内部依赖，只输出查询链路、事实源、临时索引和后端边界说明。
...
19. `receipts_render.py` 只负责日结整页 HTML shell，可以依赖 `receipts_sections.py`, `receipts_style.py` 和 `theme.py`，不能读取数据库、调用 RAG、写文件或承载 section 细节。
...
23. `route_map_render.py` 只负责三题型覆盖 HTML 渲染，可以依赖 `route_map_style.py` 和 `theme.py`，不能读取数据库或写文件。
...
27. `learning_layout.py` 可以依赖 `learning_content.py`, `learning_style.py` 和 `theme.py` 生成通用学习页 shell、卡片、题型标签和主题接入，不能保存内容、读取状态或写文件。
...
49. `web_page_sections.py` 只负责工作台首页 shell、导航、主题接入、今日闭环和只读展示区块，可以依赖 `theme.py`, `web_card_lists.py`, `web_diagnostic_lists.py`, `web_fronts.py`, `web_labels.py`, `web_page_forms.py`, `web_page_learning_forms.py`, `web_page_operations.py`, `web_page_view.py`, `web_rag_panel.py`, `web_status.py` 与 `web_page_style.py`，不能读取状态或处理 HTTP。
