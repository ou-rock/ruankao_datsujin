...
L4 报告与页面层    dashboard.py, receipts.py, receipts_payload.py, receipts_render.py, receipts_sections.py, receipts_style.py, route_map.py, route_map_payload.py, route_map_render.py, route_map_style.py, evolution.py, learning.py, learning_cheko_templates.py, learning_content.py, learning_layout.py, learning_style.py, learning_templates.py, rag.py, rag_report.py, rag_report_style.py, web_fronts.py, web_labels.py, web_lists.py, web_rag_panel.py, web_render.py
...
| `web_render.py` | L4 | `memory`, `storage`, `web_labels` | 工作台表单控件片段、状态消息和今日动作渲染。可以依赖展示标签，不能处理 HTTP、打开数据库、写状态、保存标签映射、承载列表渲染、RAG 面板或三题型概览。 |
...
| `web_page_forms.py` | L5 | `web_lists`, `web_page_view`, `web_render` | 工作台首页表单区块适配器。负责练习、学习回合、三源、记忆卡、原则网络、Vault 等表单 HTML。不解析 HTTP 或写状态。 |
| `web_page_sections.py` | L5 | `web_fronts`, `web_lists`, `web_page_forms`, `web_page_style`, `web_page_view`, `web_rag_panel`, `web_render` | 工作台首页结构适配器。负责首页 HTML shell、导航、今日闭环和只读展示区块。不读取数据库或处理 HTTP。 |
...
web_page_sections -> web_fronts, web_lists, web_page_forms, web_page_style, web_page_view, web_rag_panel, web_render
web_page_style -> -
web_page_view -> -
web_labels   -> domain
...
调用 `web_fronts.py` 题型片段、`web_rag_panel.py` RAG 面板、`web_render.py` 控制片段和 `web_lists.py` 列表片段，组合首页 HTML shell、导航、今日闭环和只读展示区块；
它不能读取数据库、构造 RAG brief 或触发写入动作。

`web_page_forms.py` 是首页表单区块适配器。它接收 `HomePageView`，
渲染工作台的写入表单结构，并可调用 `web_lists.py` 展示相关列表，但不解析 HTTP 表单、不调用写入动作。
...
- `web_render.py` 只生成工作台表单控件、消息和今日动作 HTML 片段，不写文件。
...
| `web_page_sections.py` | 约 130 行，依赖 7 个内部模块 | 页面壳已变薄，仍聚合首页所有区块调用 | 保持只做布局编排，不回填表单细节 |
...
| `web_render.py` | 约 140 行，依赖 3 个内部模块 | 控制片段、状态消息和今日动作仍集中，但 RAG、题型与列表已外移 | 后续按消息/控制继续拆 |
...
28. `web_fronts.py` 只负责工作台三题型概览计算和题型卡片 HTML 片段，可以依赖只读数据对象和 `web_labels.py`，不能处理 HTTP、写状态、打开数据库、承载页面 shell 或调用 RAG。
29. `web_lists.py` 只负责工作台列表 HTML 片段，可以依赖只读数据对象和 `web_labels.py`，不能处理 HTTP、写状态、打开数据库或承载表单区块。
30. `web_rag_panel.py` 只负责工作台 RAG 控制面板 HTML 片段，可以依赖 `web_labels.py`，不能执行检索、读取状态、写状态、打开数据库、生成 RAG brief 或承载页面 shell。
31. `web_render.py` 只负责工作台表单控件片段、状态消息和今日动作，可以依赖只读数据对象和 `web_labels.py`，不能处理 HTTP、写状态、保存标签映射、承载列表渲染、RAG 面板或计算三题型概览。
32. `web_forms.py` 只负责工作台表单字段到 typed input 的转换，可以依赖 `domain.py`，不能依赖存储或渲染。
33. `web_handlers.py` 只负责 HTTP handler、路由分发和响应封装，可以依赖 `web_forms.py`，不能依赖业务模块或存储。
34. `web_actions.py` 只负责工作台副作用动作，可以依赖 `web_forms.py` 和内层服务，不能处理 HTTP 或渲染页面。
35. `web_bootstrap.py` 只负责工作台启动初始化，可以依赖 `learning.py`, `storage.py`, `vault.py`，不能处理 HTTP 或运行时业务。
36. `web_files.py` 只负责工作台文件读取和路径防护，不能打开数据库、渲染页面或处理 HTTP 状态码。
37. `web_page_style.py` 只保存工作台首页 CSS 常量，不能读取状态、生成表单或依赖业务模块。
38. `web_page_view.py` 只定义首页 view model，不渲染 HTML、不读取状态。
39. `web_page_forms.py` 只负责工作台首页表单区块 HTML，可以依赖 `web_lists.py`, `web_page_view.py` 和 `web_render.py`，不能解析 HTTP、读取数据库或触发写入动作。
40. `web_page_sections.py` 只负责工作台首页 shell、导航、今日闭环和只读展示区块，可以依赖 `web_fronts.py`, `web_lists.py`, `web_page_forms.py`, `web_page_view.py`, `web_rag_panel.py`, `web_render.py` 与 `web_page_style.py`，不能读取状态或处理 HTTP。
41. `web_page.py` 只负责工作台首页 view model 装配，可以依赖只读内层服务，不能处理 HTTP、写状态或执行启动初始化。
42. `web_app.py` 是工作台应用编排层，可以依赖内层模块，但不能处理 HTTP server 绑定。
43. `cli.py` 和 `web.py` 是组合根，可以依赖内层模块。
44. 除 `cli.py` 之外，任何模块不得依赖 `web.py`。
45. 内层模块不得依赖 `cli.py`, `web.py`, `web_actions.py`, `web_app.py`, `web_bootstrap.py`, `web_files.py`, `web_forms.py`, `web_handlers.py`, `web_page.py`, `web_page_forms.py`, `web_page_sections.py`, `web_page_style.py`, `web_page_view.py`, `.codex` 命令、shell scripts 或浏览器自动化工具。

## 下一轮架构优化顺序

1. 如果 `web_page_style.py` 需要继续演进，再考虑拆为静态 CSS 资源。
2. 如果 `web_render.py` 继续增长，再按消息/控制拆片段模块。
