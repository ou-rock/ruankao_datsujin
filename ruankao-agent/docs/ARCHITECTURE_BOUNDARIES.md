...
L4 报告与页面层    dashboard.py, receipts.py, receipts_payload.py, receipts_render.py, receipts_sections.py, receipts_style.py, route_map.py, route_map_payload.py, route_map_render.py, route_map_style.py, evolution.py, learning.py, learning_cheko_templates.py, learning_content.py, learning_layout.py, learning_style.py, learning_templates.py, rag.py, rag_report.py, rag_report_style.py, web_controls.py, web_fronts.py, web_labels.py, web_lists.py, web_rag_panel.py, web_status.py
...
| `web_lists.py` | L4 | `memory`, `storage`, `web_labels` | 工作台卡片、练习、诊断和风险原因列表渲染。可以依赖只读数据对象和展示标签，不能处理 HTTP、打开数据库、写状态或承载表单区块。 |
...
| `web_page_forms.py` | L5 | `web_controls`, `web_lists`, `web_page_view` | 工作台首页记忆与知识网络表单区块适配器。负责三源、记忆卡、原则网络、Vault 等表单 HTML。不解析 HTTP 或写状态。 |
| `web_page_learning_forms.py` | L5 | `web_controls`, `web_lists`, `web_page_view` | 工作台首页学习入口表单区块适配器。负责 Cheko 学习信号、练习记录和学习回合 HTML。不解析 HTTP、不写状态、不承载记忆卡/原则/Vault 表单。 |
| `web_page_operations.py` | L5 | `web_page_view` | 工作台首页今日产物操作区块。负责日结、夜间进化、三题型覆盖图、RAG 简报和状态导出按钮 HTML。不解析 HTTP、不触发动作、不读取数据库。 |
| `web_page_sections.py` | L5 | `web_fronts`, `web_labels`, `web_lists`, `web_page_forms`, `web_page_learning_forms`, `web_page_operations`, `web_page_style`, `web_page_view`, `web_rag_panel`, `web_status` | 工作台首页结构适配器。负责首页 HTML shell、导航、今日闭环和只读展示区块。不读取数据库或处理 HTTP。 |
...
web_page_forms -> web_controls, web_lists, web_page_view
web_page_learning_forms -> web_controls, web_lists, web_page_view
web_page_operations -> web_page_view
web_page_sections -> web_fronts, web_labels, web_lists, web_page_forms, web_page_learning_forms, web_page_operations, web_page_style, web_page_view, web_rag_panel, web_status
web_page_style -> -
web_page_view -> -
web_labels   -> domain
...
- `web_page_learning_forms.py` 出度 3。
- `web_page_operations.py` 出度 1。
- `web_page_sections.py` 出度 10。
...
- `web_lists.py` 只生成工作台列表 HTML 片段，不写文件。
...
| `web_page_learning_forms.py` | 约 120 行，依赖 3 个内部模块 | 学习信号、练习记录和学习回合入口独立，直接支撑学习模式 | 保持只做学习入口表单，不回填记忆卡或原则表单 |
| `web_page_operations.py` | 约 40 行，依赖 1 个内部模块 | 今日产物生成按钮独立，风险低 | 保持只做操作表单 HTML，不触发动作 |
| `web_page_sections.py` | 约 133 行，依赖 10 个内部模块 | 页面壳已变薄，仍聚合首页所有区块调用 | 保持只做布局编排，不回填表单细节 |
...
| `web_lists.py` | 约 115 行，依赖 3 个内部模块 | 工作台列表渲染独立，风险中低 | 后续如继续增长，再拆卡片/练习/诊断列表 |
...
28. `web_controls.py` 只负责工作台表单控件 HTML 片段，不能读取状态、处理 HTTP、写状态、打开数据库、保存标签映射或承载表单区块布局。
29. `web_fronts.py` 只负责工作台三题型概览计算和题型卡片 HTML 片段，可以依赖只读数据对象和 `web_labels.py`，不能处理 HTTP、写状态、打开数据库、承载页面 shell 或调用 RAG。
30. `web_lists.py` 只负责工作台列表 HTML 片段，可以依赖只读数据对象和 `web_labels.py`，不能处理 HTTP、写状态、打开数据库或承载表单区块。
31. `web_rag_panel.py` 只负责工作台 RAG 控制面板 HTML 片段，可以依赖 `web_labels.py`，不能执行检索、读取状态、写状态、打开数据库、生成 RAG brief 或承载页面 shell。
32. `web_status.py` 只负责工作台状态消息、状态摘要和今日动作，可以依赖只读数据对象和 `web_labels.py`，不能处理 HTTP、写状态、保存标签映射、承载表单控件、列表渲染、RAG 面板或计算三题型概览。
33. `web_forms.py` 只负责工作台表单字段到 typed input 的转换，可以依赖 `domain.py`，不能依赖存储或渲染。
34. `web_handlers.py` 只负责 HTTP handler、路由分发和响应封装，可以依赖 `web_forms.py`，不能依赖业务模块或存储。
35. `web_actions.py` 只负责工作台副作用动作，可以依赖 `web_forms.py` 和内层服务，不能处理 HTTP 或渲染页面。
36. `web_bootstrap.py` 只负责工作台启动初始化，可以依赖 `learning.py`, `storage.py`, `vault.py`，不能处理 HTTP 或运行时业务。
37. `web_files.py` 只负责工作台文件读取和路径防护，不能打开数据库、渲染页面或处理 HTTP 状态码。
38. `web_page_style.py` 只保存工作台首页 CSS 常量，不能读取状态、生成表单或依赖业务模块。
39. `web_page_view.py` 只定义首页 view model，不渲染 HTML、不读取状态。
40. `web_page_forms.py` 只负责工作台首页三源、记忆卡、原则网络和 Vault 表单区块 HTML，可以依赖 `web_controls.py`, `web_lists.py` 和 `web_page_view.py`，不能解析 HTTP、读取数据库、触发写入动作、承载学习入口或今日产物操作。
41. `web_page_learning_forms.py` 只负责工作台首页 Cheko 学习信号、练习记录和学习回合区块 HTML，可以依赖 `web_controls.py`, `web_lists.py` 和 `web_page_view.py`，不能解析 HTTP、读取数据库、触发写入动作、承载记忆卡/原则/Vault 表单或今日产物操作。
42. `web_page_operations.py` 只负责工作台首页今日产物操作区块 HTML，可以依赖 `web_page_view.py`，不能解析 HTTP、读取数据库、触发写入动作或承载学习/记忆表单。
43. `web_page_sections.py` 只负责工作台首页 shell、导航、今日闭环和只读展示区块，可以依赖 `web_fronts.py`, `web_labels.py`, `web_lists.py`, `web_page_forms.py`, `web_page_learning_forms.py`, `web_page_operations.py`, `web_page_view.py`, `web_rag_panel.py`, `web_status.py` 与 `web_page_style.py`，不能读取状态或处理 HTTP。
44. `web_page.py` 只负责工作台首页 view model 装配，可以依赖只读内层服务，不能处理 HTTP、写状态或执行启动初始化。
45. `web_app.py` 是工作台应用编排层，可以依赖内层模块，但不能处理 HTTP server 绑定。
46. `cli.py` 和 `web.py` 是组合根，可以依赖内层模块。
47. 除 `cli.py` 之外，任何模块不得依赖 `web.py`。
48. 内层模块不得依赖 `cli.py`, `web.py`, `web_actions.py`, `web_app.py`, `web_bootstrap.py`, `web_files.py`, `web_forms.py`, `web_handlers.py`, `web_page.py`, `web_page_forms.py`, `web_page_learning_forms.py`, `web_page_operations.py`, `web_page_sections.py`, `web_page_style.py`, `web_page_view.py`, `.codex` 命令、shell scripts 或浏览器自动化工具。

## 下一轮架构优化顺序

1. 如果 `web_page_style.py` 需要继续演进，再考虑拆为静态 CSS 资源。
2. 如果 `web_status.py` 继续增长，再按消息/今日动作拆分。
3. 如果 `web_lists.py` 继续增长，再拆卡片/练习/诊断列表。
4. 如果 `web_fronts.py` 引入更多题型策略，再拆题型状态策略。
5. 如果 `rag_progress.py` 继续增长，再按记忆/题型/练习拆 gate 策略。
