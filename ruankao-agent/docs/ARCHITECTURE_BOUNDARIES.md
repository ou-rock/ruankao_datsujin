...
| `web_page_sections.py` | L5 | `web_fronts`, `web_labels`, `web_lists`, `web_page_forms`, `web_page_style`, `web_page_view`, `web_rag_panel`, `web_status` | 工作台首页结构适配器。负责首页 HTML shell、导航、今日闭环和只读展示区块。不读取数据库或处理 HTTP。 |
...
web_page_sections -> web_fronts, web_labels, web_lists, web_page_forms, web_page_style, web_page_view, web_rag_panel, web_status
web_page_style -> -
web_page_view -> -
web_labels   -> domain
...
41. `web_page_sections.py` 只负责工作台首页 shell、导航、今日闭环和只读展示区块，可以依赖 `web_fronts.py`, `web_labels.py`, `web_lists.py`, `web_page_forms.py`, `web_page_view.py`, `web_rag_panel.py`, `web_status.py` 与 `web_page_style.py`，不能读取状态或处理 HTTP。
