...
```text
L5 入口适配层      cli.py, web.py, web_handlers.py, web_forms.py, shell scripts, .codex commands
...
| `web.py` | L5 | 多数模块, `web_forms`, `web_handlers`, `web_render` | 本地 HTTP 工作台组合根。负责服务绑定、应用编排和页面组合，不成为事实源。 |
...
vault        -> -
web          -> cheko, dashboard, domain, evolution, export_state, learning, loop, memory, rag, receipts, route_map, storage, study, vault, web_forms, web_handlers, web_render
web_forms    -> domain
...
- `cheko.py`, `principles.py`, `study.py`, `web.py` 可以通过 `RuankaoStore` 写入。
...
| 热点 | 证据 | 风险 | 优化方向 |
| --- | --- | --- | --- |
| `web.py` | 约 1300 行，依赖 17 个内部模块 | 页面组合和应用服务仍在一起 | 已拆出 `web_render.py`, `web_forms.py`, `web_handlers.py`；后续继续拆 `web_app.py` |
...
8. `cli.py` 和 `web.py` 是组合根，可以依赖内层模块。
9. 除 `cli.py` 之外，任何模块不得依赖 `web.py`。
10. 内层模块不得依赖 `cli.py`, `web.py`, `web_forms.py`, `web_handlers.py`, `.codex` 命令、shell scripts 或浏览器自动化工具。

## 下一轮架构优化顺序

1. 继续拆 `web.py`：`web_render.py` 已承接 HTML 片段，`web_forms.py` 已承接表单输入，`web_handlers.py` 已承接路由处理，下一步拆应用服务。
