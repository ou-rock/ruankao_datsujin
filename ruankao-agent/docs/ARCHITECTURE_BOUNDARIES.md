...
```text
L5 入口适配层      cli.py, web.py, web_app.py, web_actions.py, web_bootstrap.py, web_files.py, web_handlers.py, web_forms.py, web_page.py, web_page_sections.py, web_page_style.py, shell scripts, .codex commands
...
| `web_page_sections.py` | L5 | `web_page_style`, `web_render` | 工作台首页结构适配器。负责首页 HTML shell、导航、区块和表单结构。不读取数据库或处理 HTTP。 |
| `web_page.py` | L5 | `domain`, `evolution`, `export_state`, `memory`, `rag`, `receipts`, `route_map`, `web_page_sections`, `web_render` | 工作台首页数据适配器。负责首页只读数据装配和 view model，不处理 HTTP、写入动作或启动初始化。 |
...
web_page     -> domain, evolution, export_state, memory, rag, receipts, route_map, web_page_sections, web_render
...
调用 `web_render.py` 片段并组合首页 HTML shell、导航、区块和表单；
...
- `web_page_sections.py` 组合工作台首页 HTML，不写文件。
...
| 热点 | 证据 | 风险 | 优化方向 |
| --- | --- | --- | --- |
| `web_page_sections.py` | 约 420 行，依赖 2 个内部模块 | 首页 HTML shell、导航、区块和表单结构仍在一个模块 | 后续按区块拆 `web_page_forms.py` 或 `web_page_today.py` |
| `web_page.py` | 约 130 行，依赖 9 个内部模块 | 首页数据装配仍集中，但结构已外移 | 保持只做 view model 装配 |
...
12. `web_page_sections.py` 只负责工作台首页结构和表单 HTML，可以依赖 `web_render.py` 与 `web_page_style.py`，不能读取状态或处理 HTTP。
13. `web_page.py` 只负责工作台首页 view model 装配，可以依赖只读内层服务，不能处理 HTTP、写状态或执行启动初始化。
14. `web_app.py` 是工作台应用编排层，可以依赖内层模块，但不能处理 HTTP server 绑定。
15. `cli.py` 和 `web.py` 是组合根，可以依赖内层模块。
16. 除 `cli.py` 之外，任何模块不得依赖 `web.py`。
17. 内层模块不得依赖 `cli.py`, `web.py`, `web_actions.py`, `web_app.py`, `web_bootstrap.py`, `web_files.py`, `web_forms.py`, `web_handlers.py`, `web_page.py`, `web_page_sections.py`, `web_page_style.py`, `.codex` 命令、shell scripts 或浏览器自动化工具。

## 下一轮架构优化顺序

1. 继续拆 `web_page_sections.py`：首页结构已离开数据装配模块，下一步按表单/今日闭环区块拆页面结构。
