...
```text
L5 入口适配层      cli.py, web.py, web_app.py, web_actions.py, web_bootstrap.py, web_files.py, web_handlers.py, web_forms.py, web_page.py, web_page_forms.py, web_page_operations.py, web_page_sections.py, web_page_style.py, web_page_view.py, shell scripts, .codex commands
...
| `web_page_sections.py` | L5 | `web_fronts`, `web_labels`, `web_lists`, `web_page_forms`, `web_page_operations`, `web_page_style`, `web_page_view`, `web_rag_panel`, `web_status` | 工作台首页结构适配器。负责首页 HTML shell、导航、今日闭环和只读展示区块。不读取数据库或处理 HTTP。 |
...
web_page_sections -> web_fronts, web_labels, web_lists, web_page_forms, web_page_operations, web_page_style, web_page_view, web_rag_panel, web_status
web_page_style -> -
web_page_view -> -
web_labels   -> domain
...
| 热点 | 证据 | 风险 | 优化方向 |
| --- | --- | --- | --- |
| `web_page_forms.py` | 约 280 行，依赖 3 个内部模块 | 表单区块已经离开页面壳和今日产物操作，但仍承载多种写入入口 | 后续如继续增长，再按练习/记忆/同步拆小 |
| `web_page_operations.py` | 约 40 行，依赖 1 个内部模块 | 今日产物生成按钮独立，风险低 | 保持只做操作表单 HTML，不触发动作 |
| `web_page_sections.py` | 约 130 行，依赖 9 个内部模块 | 页面壳已变薄，仍聚合首页所有区块调用 | 保持只做布局编排，不回填表单细节 |
...
40. `web_page_forms.py` 只负责工作台首页学习/记录/记忆/原则/Vault 表单区块 HTML，可以依赖 `web_controls.py`, `web_lists.py` 和 `web_page_view.py`，不能解析 HTTP、读取数据库、触发写入动作或承载今日产物操作。
41. `web_page_operations.py` 只负责工作台首页今日产物操作区块 HTML，可以依赖 `web_page_view.py`，不能解析 HTTP、读取数据库、触发写入动作或承载学习/记忆表单。
42. `web_page_sections.py` 只负责工作台首页 shell、导航、今日闭环和只读展示区块，可以依赖 `web_fronts.py`, `web_labels.py`, `web_lists.py`, `web_page_forms.py`, `web_page_operations.py`, `web_page_view.py`, `web_rag_panel.py`, `web_status.py` 与 `web_page_style.py`，不能读取状态或处理 HTTP。
43. `web_page.py` 只负责工作台首页 view model 装配，可以依赖只读内层服务，不能处理 HTTP、写状态或执行启动初始化。
44. `web_app.py` 是工作台应用编排层，可以依赖内层模块，但不能处理 HTTP server 绑定。
45. `cli.py` 和 `web.py` 是组合根，可以依赖内层模块。
46. 除 `cli.py` 之外，任何模块不得依赖 `web.py`。
47. 内层模块不得依赖 `cli.py`, `web.py`, `web_actions.py`, `web_app.py`, `web_bootstrap.py`, `web_files.py`, `web_forms.py`, `web_handlers.py`, `web_page.py`, `web_page_forms.py`, `web_page_operations.py`, `web_page_sections.py`, `web_page_style.py`, `web_page_view.py`, `.codex` 命令、shell scripts 或浏览器自动化工具。
...
9. 如果工作台表单继续增长，再拆 `web_page_forms.py` 的练习/记忆/同步区块。
